...
I'm using the formula $\hat{\beta}=(X^tX)^{-1}X^ty$

```{r}
# 1. Find out what the analytical OLS solution is. Think it's formula (8.2) that I need to use (check)
# a. find out how to transpose X into another matrix in R
Xtrans <- t(X)
# b. find out to take the inverse of X^tX and put into another matrix
# c. Multiply (X^tX)^-X^ty into a matrix
# d. Find the estimates for beta_0 and beta_1 and put into above graph
bhat <- solve(Xtrans%*%X)%*%Xtrans%*%y
...
```{r}
# I assume that there's a total of 256 data points spread out between x = 15 and x = 300
# 1. Generate uniformly distributed from x = 15 to x = 300 into a variable x
x <- runif(256, min = 15, max = 300)

# 2. generate y values b[1]+b[2]x + rnorm(256, mean = 0, sd = sigma) into variable y
y <- b[1]+b[2]*x + rnorm(256, mean = 0, sd = sigma)

# 3. put x and y values into a tibble data frame

GPAInc <- tibble(y,x)

# 4. plot x and y values on a scatterplot (same code as earlier)
# 5. add regressionline to plot (there's code earlier that shows how)
GPAInc %>% ggplot(aes(x = x, y = y)) +
    geom_point(colour = colours[2]) +
    geom_abline(intercept = b[1], slope = b[2])+
    theme_light()+
    labs()
...
fit1 <- stan_glm(y~x, data = GPAInc)
# 2. Extract values for b_0 and b_1 using summary() function
summary(GPAInc_glm)
# 3. Copy code for plot from untop and add a regssion line
GPAInc %>% ggplot(aes(x = x, y = y)) +
    geom_point(colour = colours[2]) +
...
```{r}
mcmc_areas(fit1, pars = "(Intercept)", prob = 0.50, prob_outer = 1)
...
# By hand. 
print(fit1$coefficients[1]+fit1$coefficients[2]*40)

print(fit1$coefficients[1]+fit1$coefficients[2]*80)
```
...
```{r}
# 1. Discussing mean difference
# It's not suprising that ON AVERAGE the mean student from a 80 household 
q <- quantile(diff$mean_diff, c(0.025, 0.975))
print(q)

# 2. discussing individual difference 

#not suprising either that individuals who lives in a house of 40k per year can score higher than someone from an 80k household. G
f <- quantile(diff$indiv_diff, c(0.025, 0.975))
print(f)


hist(diff$indiv_diff)
```
...
```{r}
# step 1. find out how many percent of the values indiv_diff  (y_pred[,2] - y_pred[,1]) is above 0. 

# a) nrow(diff[diff$indiv_diff>0, ]) / 

nrow(diff[diff$indiv_diff>0, ])/nrow(diff)*100

# 63.95 procent chance. 
```
...
```{r}
nrow(diff[diff$mean_diff>0, ])/nrow(diff)*100
```
